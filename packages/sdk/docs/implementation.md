...
文法の構成、解析、ノードの型と意味検証は Rust にあります。`crates/grammar/src/bindings/presets.rs` の一覧が配布する文法名・native grammar・編集用 builder を所有し、同じ一覧から TypeScript / Go のプリセット名を生成します。`crates/wasm/src/node_types.rs` に配布するノード契約を登録します。SDK の文法選択は native grammar のキャッシュを直接共有します。編集可能な Catalog API は引き続き利用できます。

Rust API は [parser core](../../core/crates/parser/README.md) と [traQ presets](../crates/grammar/README.md) を参照してください。ネイティブ Rust では引き続き Plugin / Rule の追加・削除・並べ替えを利用できます。
...
core の `/renderer` は受け取った Document から HTML を作ります。レンダラーの Plugin 宣言・登録・合成も core が所有します。構文別の handler は commonmark-plugin と traq-plugin、traQ の構成と condensed 表示は SDK の `/renderer` が所有します。文法を構成する Rust の Plugin とは別の API です。

通知・参照抽出のネイティブ Rust API は [processing](https://github.com/uni-kakurenbo/traq-markdown-engine/tree/main/packages/sdk/crates/processing) にあります。保存済みメッセージの文法版は利用側で管理し、原文を対応するプリセットで再解析します。永続 AST の互換層は設けません。
...
implementations remain in core/commonmark-plugin/traq-plugin.
