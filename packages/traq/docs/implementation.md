...
通知・参照抽出のネイティブ Rust API は [processing](https://github.com/uni-kakurenbo/traq-markdown-engine/packages/traq/tree/main/crates/processing) にあります。保存済みメッセージの文法版は利用側で管理し、原文を対応するプリセットで再解析します。永続 AST の互換層は設けません。
